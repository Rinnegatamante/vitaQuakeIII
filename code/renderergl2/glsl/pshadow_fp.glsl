...
uniform float3      u_LightUp;
uniform float3      u_LightRight;
...
void main(
	...
	float3 lightToPos = var_Position - u_LightOrigin.xyz;
	float2 st = float2(-dot(u_LightRight, lightToPos), dot(u_LightUp, lightToPos));
	...
	float part;
#if defined(USE_PCF)
	part  = float(tex2D(u_ShadowMap, st + float2(-1.0/512.0, -1.0/512.0)).r != 1.0);
	part += float(tex2D(u_ShadowMap, st + float2( 1.0/512.0, -1.0/512.0)).r != 1.0);
	part += float(tex2D(u_ShadowMap, st + float2(-1.0/512.0,  1.0/512.0)).r != 1.0);
	part += float(tex2D(u_ShadowMap, st + float2( 1.0/512.0,  1.0/512.0)).r != 1.0);
#else
	part  = float(tex2D(u_ShadowMap, st).r != 1.0);
#endif
