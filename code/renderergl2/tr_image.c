...
static void RawImage_UploadTexture(GLuint texture, byte *data, int x, int y, int width, int height, GLenum target, GLenum picFormat, int numMips, GLenum internalFormat, imgType_t type, imgFlags_t flags, qboolean subtexture )
{
	...
	qboolean rgtc = qfalse;
	qboolean rgba8 = picFormat == GL_RGBA8;
	qboolean rgba = rgba8;
	qboolean mipmap = !!(flags & IMGFLAG_MIPMAP);
	int size, miplevel;
	...
	do
	{
...
#else
			printf("WARNING: glCompressedTextureSubImage2D called!\n");
#endif
		}
		else
		{
			...
			if (rgba8 && rgtc)
				...
				glTexSubImage2D(target, miplevel, x, y, width, height, dataFormat, dataType, data);
			}
		}
		...
		if (!lastMip && numMips < 2)
		{
			...
			{
				qglGenerateTextureMipmapEXT(texture);
				break;
			}
			else if (rgba8)
			{
...
image_t *R_CreateImage2( const char *name, byte *pic, int width, int height, GLenum picFormat, int numMips, imgType_t type, imgFlags_t flags, int internalFormat ) {
	...
	glBindTexture(textureTarget, image->texnum);
	tr.numImages++;
	...
	do
	{
		...
		if (cubemap)
		{
			...
			for (i = 0; i < 6; i++)
				glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, miplevel, internalFormat, mipWidth, mipHeight, 0, dataFormat, GL_UNSIGNED_BYTE, NULL);
		}
		else
		{
			glTexImage2D(GL_TEXTURE_2D, miplevel, internalFormat, mipWidth, mipHeight, 0, dataFormat, GL_UNSIGNED_BYTE, NULL);
		}
	...
	glTexParameterf(textureTarget, GL_TEXTURE_WRAP_S, glWrapClampMode);
	glTexParameterf(textureTarget, GL_TEXTURE_WRAP_T, glWrapClampMode);
#ifndef __vita__
	if (cubemap)
	...
	switch(internalFormat)
	{
		...
		case GL_DEPTH_COMPONENT32_ARB:
			...
			break;
		default:
			glTexParameterf(textureTarget, GL_TEXTURE_MIN_FILTER, mipmap ? gl_filter_min : GL_LINEAR);
			glTexParameterf(textureTarget, GL_TEXTURE_MAG_FILTER, mipmap ? gl_filter_max : GL_LINEAR);
			break;
	}
