...
bool retro_load_game(const struct retro_game_info *info)
{
	...
	{
		if (log_cb)
			log_cb(RETRO_LOG_WARN, "vitaQuakeIII: libretro frontend doesn't have OpenGL support, falling back to TinyGL.\n");
...
void retro_run(void)
{
	if (!use_pgl) qglBindFramebuffer(RARCH_GL_FRAMEBUFFER, hw_render.get_current_framebuffer());
	...
	if (first_boot) {
		...
		if (use_pgl) Cvar_SetValue("r_noportals", 1);
		first_boot = false;
	}
...
{
	if (use_pgl) video_cb(pgl_buffer, scr_width, scr_height, scr_width << 1);
